...
for iD in range(len(lst_dim)):
    ...
    for iM in range(len(lst_m_axis)):
        ...
        for iT in range(n_trial):
            ...
            # build m-tensor
            ti()
            phi = mt.MTensor([np.array([psi(lhs[k, i]) for k in range(m)]) for i in range(dim)])
            ...
            if LR:
                # need to do a PCA for sv, SVD is not ok!!!
                ...
                # test PCA
                ti()
                sv = phi.pca(s_only=True)# doubles work in timing!!!
                U, S = phi.svd()
                print(f'time to compmute SVD of P: {tf()}s')
                QOI[iT, iD, iM, 5] = tf()
                
                if SHOW_SV:

                    plt.plot(sv, label='PCA')
                ...
                Z = U_ @ U_.T @ np.linalg.lstsq(phi@phi, rhs)[0]
                print(f'time to build Z: {tf()}s')
                QOI[iT, iD, iM, 6] = tf()
            ...
            if ALID:
                ...
                phi_ALI = mt.MTensor([psi(data_init[i]) for i in range(dim)])
                ...
                for e in lhs[1:]:
                    ...
                    phi_i = mt.MTensor([psi(e[i]) for i in range(dim)])
            ...
            def f_(x):
                ...
                phi_x = mt.MTensor([psi(x[i]) for i in range(dim)])
